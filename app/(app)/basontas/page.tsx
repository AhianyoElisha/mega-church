...
import {
  useBasontas,
  useCreateBasonta,
  useCreateBasontaCategory,
  useLeaderAccounts,
} from '@/lib/queries/groups'
import { buildBasontaTree } from '@/lib/groups/tree'
import type { BasontaWithCount } from '@/lib/groups/types'

/**
 * Basontas — the work groups members serve in.
 *
 ...
 * it standalone.
 *
 * This is what `/bacentas` used to be. The church's word for a serving group
 * is BASONTA; a bacenta is a place under a constituency. One page doing both
 * jobs is what made the vocabulary unusable, and splitting them is the point.
 */
export default function BasontasPage() {
  ...
  const { data, isLoading } = useBasontas()
  const createBasonta = useCreateBasonta()
  const createCategory = useCreateBasontaCategory()
  const leaders = useLeaderAccounts()

  const [form, setForm] = useState<'none' | 'basonta' | 'category'>('none')
  ...
  const tree = useMemo(() => {
    ...
    return buildBasontaTree(data.categories, data.basontas)
  }, [data])
  ...
  const submitBasonta = async (e: React.FormEvent) => {
    e.preventDefault()
    setError(null)
    try {
      const res = await createBasonta.mutateAsync({
        name,
      ...
      setError(err instanceof Error ? err.message : 'Could not create that basonta.')
    }
  }
  ...
  return (
    <PageWrap>
      <PageHeader
        title="Basontas"
        subtitle="The work groups members serve in — choir, technical, media. Someone can be in several at once."
        actions={
          <>
            ...
            <Button
              color="primary"
              onClick={() => setForm(form === 'basonta' ? 'none' : 'basonta')}
            >
              New basonta
            </Button>
          </>
        ...
        <Card className="mb-6">
          <form onSubmit={submitCategory} className="grid gap-4">
            ...
            <p className="-mt-2 text-sm text-neutral-500 dark:text-neutral-400">
              A family of basontas, like <strong>Choir</strong> over Biazo, Living Waters and
              Fresh Oil. Nobody is a member of a category — they join one of the basontas inside
              it. If the group has no sub-groups, skip this and create a basonta on its own.
            </p>
            <Field>
              <Label>Name</Label>
              <Input
      ...
      {form === 'basonta' && (
        <Card className="mb-6">
          <form onSubmit={submitBasonta} className="grid gap-4">
            <h2 className="text-base font-semibold text-neutral-950 dark:text-white">
              New basonta
            </h2>
            <Field>
            ...
            {error && <Banner tone="error">{error}</Banner>}
            <div className="flex gap-3">
              <Button type="submit" color="primary" disabled={createBasonta.isPending}>
                {createBasonta.isPending ? 'Creating…' : 'Create basonta'}
              </Button>
              <Button type="button" plain onClick={reset}>
                Cancel
              </Button>
      ...
      {isLoading ? (
        <Card padded={false}>
          <LoadingRow label="Loading basontas…" />
        </Card>
      ) : isEmpty ? (
        <EmptyState
          icon={UserGroupIcon}
          title="No basontas yet"
          ...
          action={
            <Button color="primary" onClick={() => setForm('basonta')}>
              New basonta
            </Button>
          }
        />
        ...
        <div className="flex flex-col gap-8">
          {tree!.categories.map(({ category, basontas }) => (
            <section key={category.$id}>
              <div className="mb-3 flex items-baseline gap-3">
                ...
                <span className="text-xs text-neutral-400">
                  {basontas.length} basonta{basontas.length === 1 ? '' : 's'}
                </span>
              </div>
              ...
              {basontas.length === 0 ? (
                <Card>
                  <p className="text-sm text-neutral-500 dark:text-neutral-400">
                    Nothing in this category yet. Create a basonta and choose{' '}
                    <strong>{category.name}</strong> as its category.
                  </p>
                </Card>
              ) : (
                <BasontaGrid basontas={basontas} />
              )}
            </section>
          ))}
          ...
          {tree!.standalone.length > 0 && (
            <section>
              ...
              <BasontaGrid basontas={tree!.standalone} />
            </section>
          )}

          {/*
            An orphan is a basonta whose category was deleted out from under it.
          ...
          {tree!.orphans.length > 0 && (
            <section>
              <Banner tone="warning" className="mb-3">
                These basontas point at a category that no longer exists. Edit each one and give
                it a category, or leave it standalone.
              </Banner>
              <BasontaGrid basontas={tree!.orphans} />
            </section>
          )}
        </div>
      )}
...
function BasontaGrid({ basontas }: { basontas: BasontaWithCount[] }) {
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {basontas.map((b) => (
        ...
        >
          {/* `wrap-anywhere` on both: a basonta name is free text up to 96
