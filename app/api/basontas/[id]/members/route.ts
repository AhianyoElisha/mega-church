...
import { applyBasontaMembership, getBasonta } from '@/lib/groups/server'
import type { MembershipMode, MembershipResponse } from '@/lib/groups/types'

type Ctx = { params: Promise<{ id: string }> }

/**
 * POST — change who serves in this basonta.
 *
 ...
 * may sing in two choirs at once. That is the whole reason basonta membership
 * is a join collection rather than a field — and it is precisely why the
 * church's PLACES were the wrong thing to model this way, which is what the
 * split between bacenta and basonta exists to fix.
 *
 *   add     put these members in, leave everyone else alone   ← the assigner
 *   remove  take these members out, leave everyone else alone
 *   set     the basonta ends up being exactly these members   ← destructive
 *
...
export async function POST(request: NextRequest, { params }: Ctx) {
  ...
  const mode: MembershipMode = body.mode === 'set' || body.mode === 'remove' ? body.mode : 'add'

  // An empty list is meaningful for `set` (empty the basonta) and meaningless
  ...
  if (!(await getBasonta(databases, id))) return bad('No such basonta.', 404)

  const result = await applyBasontaMembership(databases, id, memberIds, mode, auth.user.email)
