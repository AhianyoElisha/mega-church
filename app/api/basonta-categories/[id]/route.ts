...
export async function PATCH(request: NextRequest, { params }: Ctx) {
  ...
  if (body.name !== undefined) {
    const others = (await listBasontaCategories(databases)).filter((c) => c.$id !== id)
  ...
  try {
    const category = await updateBasontaCategory(databases, id, fields)
    return NextResponse.json<BasontaCategoryResponse>({ ok: true, category })
  } catch {
...
/**
 * DELETE — refused while the category still holds basontas.
 *
 * See `deleteBasontaCategory`: orphaning them would leave real groups full of
 * real people rendering as "category missing", and only the admin knows where
 * those basontas ought to go instead.
 */
export async function DELETE(_request: NextRequest, { params }: Ctx) {
  ...
  const { id } = await params
  const { databases } = createAdminClient()
  const res = await deleteBasontaCategory(databases, id)
